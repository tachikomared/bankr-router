# Bankr Router — Debug Guide

## Common Patterns

### 1. Gateway returns `router_error` / HTTP 500

**Symptom:** `{"error":"router_error","message":"Failed to load Bankr provider catalog"}`
...
curl http://127.0.0.1:8787/v1/route -X POST \
  ...
  -d '{"model":"bankr-router/auto","messages":[{"role":"user","content":"hi"}]}'
...
**Symptom:** `{"error":"router_error","message":"No configured tier models exist in current BANKR catalog"}`

**Cause:** Your Bankr provider catalog doesn’t include any models in the tier chain.

**Fix:**
- Confirm your `~/.openclaw/config.json` has a `models.providers.bankr.models` array
...
**Known cause:** Bankr Router `v0.8.1` has aggressive abort handling. Fixed in `v0.8.2`.
...
```bash
cd ~/tachi/workspace/openclawbankrrouter
