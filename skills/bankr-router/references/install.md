# Bankr Router — Install

## Prerequisites

- OpenClaw gateway installed and running
- Bankr provider configured in your `~/.openclaw/config.json`
...
3. **Verify it's up**
   ...
   Expected: `{"ok":true,"name":"bankr-router","upstream":"https://llm.bankr.bot/v1",...}`
...
```bash
cd ~/tachi/workspace/openclawbankrrouter
docker build -t bankr-router .
docker run -d -p 8787:8787 \
  -e OPENCLAW_CONFIG_PATH=/root/.openclaw/config.json \
  -v ~/.openclaw:/root/.openclaw \
  --name bankr-router bankr-router
```
